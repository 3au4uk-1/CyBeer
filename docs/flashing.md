...
| Раздел | Назначение |
|--------|------------|
| `otadata` | Служебный раздел OTA (какой слот загружать) — **обязателен** |
| `factory` / `ota_0` / `ota_1` | Приложение (по 0x140000 байт), OTA |
| `littlefs` | Веб-статика (`www`) |
| `nvs` | Wi-Fi, PIN, число LED и т.д. |

> **Важно:** если OTA пишет `not found otadata`, прошейте устройство по USB полной командой `idf.py flash` (не только `app-flash`) — нужно записать новую таблицу разделов.
